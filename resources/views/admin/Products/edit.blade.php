            ...
            <div id="specs-container" class="space-y-2">
                @if(old('specs'))
                    @foreach(old('specs') as $index => $spec)
                        <div class="flex gap-2 spec-row items-center">
                            <input type="text" name="specs[{{ $index }}][name]" value="{{ $spec['name'] }}" placeholder="Tên thông số (VD: Điện áp)" class="w-1/2 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 bg-white" required>
                            <input type="text" name="specs[{{ $index }}][value]" value="{{ $spec['value'] }}" placeholder="Giá trị (VD: 690 VAC)" class="w-1/2 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 bg-white" required>
                    ...
                    @endforeach
                @elseif($product->specifications && $product->specifications->count() > 0)
                    @foreach($product->specifications as $index => $spec)
                        <div class="flex gap-2 spec-row items-center">
                            <input type="text" name="specs[{{ $index }}][name]" value="{{ $spec->name }}" placeholder="Tên thông số (VD: Điện áp)" class="w-1/2 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 bg-white" required>
                            <input type="text" name="specs[{{ $index }}][value]" value="{{ $spec->pivot->value }}" placeholder="Giá trị (VD: 690 VAC)" class="w-1/2 px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 bg-white" required>
                            <button type="button" class="remove-spec-btn px-3 py-2 bg-red-500 hover:bg-red-600 text-white rounded-md text-sm transition-colors cursor-pointer">Xóa</button>
                        </div>
                    @endforeach
                @endif
